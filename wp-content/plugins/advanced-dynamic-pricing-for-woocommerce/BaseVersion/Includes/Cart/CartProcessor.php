...
class CartProcessor {
	...
	public function installActionFirstProcess() {
		...
		$this->cartCouponsProcessor->setFilterToReplaceCouponMessageIfDisableCoupons();
		...
		if ( $result ) {
			...
			foreach ( $freeProductsMapping as $loopCartItemKey => $freeItems ) {
				foreach ( $freeItems as $freeItem ) {
					...
					$facade = new WcCartItemFacade( $this->context, $clonedWcCart->cart_contents[ $loopCartItemKey ], $loopCartItemKey );
					...
					$newFacade = new WcCartItemFacade( $this->context, $wcCart->cart_contents[ $cartItemKey ], $cartItemKey );
	...
	protected function addCommonItems( $cart, $wcCart ) {
		...
		foreach ( $items as $item ) {
			...
			$facade->setOriginalPrice( $facade->getProduct()->get_price( 'edit' ) );
			$productPrice = $this->overrideCentsStrategy->maybeOverrideCents( $productPrice, $item );
			...
			if ( in_array( $facade->getKey(), $processedItemKeys ) ) {
				...
				if ( isset( $wcCart->cart_contents[ $cart_item_key ] ) ) {
					$alreadyProcessedItemFacade = new WcCartItemFacade( $this->context, $wcCart->cart_contents[ $cart_item_key ], $cart_item_key );
	...
	protected function maybeRemoveOriginCoupons( $cart, $wcCart ) {
		if ( $this->context->get_option( 'external_coupons_behavior' ) === 'disable_if_any_rule_applied' ) {
			$cart->removeAllOriginCoupon();
		} elseif ( $this->context->get_option( 'external_coupons_behavior' ) === 'disable_if_any_of_cart_items_updated' ) {
			...
			if ( $is_price_changed ) {
				...
				$this->replaceWcNotice(
					array(
						'text' => __( 'Sorry, coupons are disabled for these products.', 'advanced-dynamic-pricing-for-woocommerce' ),
						'type' => 'error',
					),
					array(
