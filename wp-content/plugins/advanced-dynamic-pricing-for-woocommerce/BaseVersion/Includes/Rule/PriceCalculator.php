	...
	public function applyItemDiscount( &$item, &$cart, $handler ) {
		...
		if ( $handler->isReplaceWithCartAdjustment() ) {
			...
			if ( $amount > 0 ) {
				...
				$taxClass = $globalContext->get_is_prices_include_tax() ? "" : "standard";
				$cart->addFee( new Fee( $globalContext, Fee::TYPE_ITEM_OVERPRICE, $adjustmentCode, ( - 1 ) * $amount, $taxClass, $this->rule->getId() ) );
			}
