...
class GiftStrategy {
	...
	public function addCartItemGifts( &$cart, $collection ) {
		...
		while ( $index < $attemptCount ) {
			...
			foreach ( $rule->getItemGiftsCollection()->asArray() as $gift ) {
				...
				$hash                = $freeCartItemChoices->generateHash( $rule, $giftIndex, $gift->isAllowToSelect() );
		...
		$customer = $cart->get_context()->getCustomer();
	...
	public function addCartSetGifts( &$cart, $collection ) {
		...
		while ( $index < $attemptCount ) {
			...
			foreach ( $rule->getItemGiftsCollection()->asArray() as $gift ) {
				...
				$freeCartItemChoices = $this->convertGiftToFreeCartItemChoices( $gift, $set->get_items() );
				$hash                = $freeCartItemChoices->generateHash( $rule, $giftIndex, $gift->isAllowToSelect() );
	 ...
	 */
	protected function prepareFreeCartItem( $cart, $productId, $qty, $associatedGiftHash, $variation = null, $cartItemData = null ) {
	...
	protected function processFreeItem( &$freeCartItemsByHash, $hash, $cart, $freeCartItemChoices ) {
