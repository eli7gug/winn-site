	...
	protected function make_get_rules_args() {
		...
		if ( ! empty( $_GET['product'] ) ) {
			...
			if ( ! empty( $_GET['product_childs'] ) && is_array( $_GET['product_childs'] ) ) {
				$args['product_childs'] = array_map( function( $value ) { return (int)$value; }, $_GET['product_childs'] );
			}
			if ( ! empty( $_GET['product_categories'] ) && is_array( $_GET['product_categories'] ) ) {
				$args['product_categories'] = array_map( function( $value ) { return (int)$value; }, $_GET['product_categories'] );
			}
			if ( ! empty( $_GET['product_category_slug'] ) && is_array( $_GET['product_category_slug'] ) ) {
				$args['product_category_slug'] = array_map( function( $value ) { return sanitize_text_field( (string)$value ); }, $_GET['product_category_slug'] );
			}
			if ( ! empty( $_GET['product_attributes'] ) && is_array( $_GET['product_attributes'] ) ) {
				$args['product_attributes'] = array_map( function( $value ) { return (int)$value; }, $_GET['product_attributes'] );
			}
			if ( ! empty( $_GET['product_tags'] ) && is_array( $_GET['product_tags'] ) ) {
				$args['product_tags'] = array_map( function( $value ) { return (int)$value; }, $_GET['product_tags'] );
			}
