...
class PriceAjax {
	...
	public function register() {
		...
		add_action( "wp_ajax_nopriv_" . self::ACTION_CALCULATE_SEVERAL_PRODUCTS, array( $this, "ajaxCalculateSeveralProducts" ) );
	...
	public function ajaxCalculatePrice() {
		...
		$result = $this->calculatePrice( $prodId, $qty, $customPrice );
	...
	public function ajaxCalculateSeveralProducts() {
		...
		foreach ( $readyList as $item ) {
			$result[ $item['product_id'] ] = $this->calculatePrice( $item['product_id'], $item['qty'], $item['custom_price'] );
		}
	 ...
	 * @return array|null
	 */
	protected function calculatePrice( $productId, $qty, $customPrice = null ) {
			...
			return array(
				...
				'subtotal_html' => $isOnSale ?
					$this->priceFunctions->formatSalePrice(
						$this->priceFunctions->getPriceToDisplay( $product,	array( 'qty' => $qty, 'price' => $regularPrice ) ),
		...
		if ( $processedProduct instanceof ProcessedProductSimple) {
			...
			return array(
				...
				'original_price'      => $this->priceFunctions->getPriceToDisplay( $product, array( 'price' => $processedProduct->getOriginalPrice() ) ),
				'discounted_price'    => $this->priceFunctions->getPriceToDisplay( $product, array( 'price' => $processedProduct->getCalculatedPrice() ) ),
				'original_subtotal'   => $this->priceFunctions->getPriceToDisplay( $product, array( 'price' => $processedProduct->getOriginalPrice(), 'qty' => $qty ) ),
				'discounted_subtotal' => $this->priceFunctions->getPriceToDisplay( $product, array( 'price' => $processedProduct->getCalculatedPrice(), 'qty' => $qty ) ),
			);
			...
			return array(
				...
				'lowest_original_price'       => $this->priceFunctions->getPriceToDisplay( $lowestOrigPriceProduct->getProduct(), array( 'price' => $lowestOrigPriceProduct->getOriginalPrice() ) ),
				'highest_original_price'      => $this->priceFunctions->getPriceToDisplay( $highestOrigPriceProduct->getProduct(), array( 'price' => $highestOrigPriceProduct->getOriginalPrice() ) ),
				'lowest_discounted_price'     => $this->priceFunctions->getPriceToDisplay( $lowestPriceProduct->getProduct(), array( 'price' => $lowestPriceProduct->getCalculatedPrice() ) ),
				'highest_discounted_price'    => $this->priceFunctions->getPriceToDisplay( $highestPriceProduct->getProduct(), array( 'price' => $highestPriceProduct->getCalculatedPrice() ) ),

				'lowest_original_subtotal'    => $this->priceFunctions->getPriceToDisplay( $lowestOrigPriceProduct->getProduct(), array( 'price' => $lowestOrigPriceProduct->getOriginalPrice(), 'qty' => $qty ) ),
				'highest_original_subtotal'   => $this->priceFunctions->getPriceToDisplay( $highestOrigPriceProduct->getProduct(), array( 'price' => $highestOrigPriceProduct->getOriginalPrice(), 'qty' => $qty ) ),
				'lowest_discounted_subtotal'  => $this->priceFunctions->getPriceToDisplay( $lowestPriceProduct->getProduct(), array( 'price' => $lowestPriceProduct->getCalculatedPrice(), 'qty' => $qty ) ),
				'highest_discounted_subtotal' => $this->priceFunctions->getPriceToDisplay( $highestPriceProduct->getProduct(), array( 'price' => $highestPriceProduct->getCalculatedPrice(), 'qty' => $qty ) ),
			);
		}
