...
class GiftCardsSomewhereWarmCmp {
	...
	public function applyCompatibility() {
		...
		if ( false === ( $priority1 = has_action( 'woocommerce_after_calculate_totals', array( $gift_cart, 'after_calculate_totals' ) ) ) ) {
			return;
		}

		if ( false === ( $priority2 = has_action( 'woocommerce_after_calculate_totals', array( 'WC_GC_Compatibility', 'decrease_cart_totals_recursive_counter' ) ) ) ) {
			return;
		}
		remove_action( 'woocommerce_after_calculate_totals', array( $gift_cart, 'after_calculate_totals' ), $priority1 );
		remove_action( 'woocommerce_after_calculate_totals', array( 'WC_GC_Compatibility', 'decrease_cart_totals_recursive_counter' ), $priority2 );
		add_action( 'woocommerce_after_calculate_totals', array( $gift_cart, 'after_calculate_totals' ), PHP_INT_MAX - 1 );
		add_action( 'woocommerce_after_calculate_totals', array( 'WC_GC_Compatibility', 'decrease_cart_totals_recursive_counter' ), PHP_INT_MAX );
	}
