...
class FreeItemsController {
	...
	public function handleRestoreDeletedItems() {
		...
		$customer         = $this->cart->get_context()->getCustomer();
		$removedFreeItems = $customer->getRemovedFreeItems( $hash );
		$removedFreeItems->purge();

		$wcSessionFacade = $this->cart->get_context()->getSession();
	...
	public function insertRemovedFreeCartItemStubInCart() {
		...
		foreach ( $this->cart->get_context()->getCustomer()->getRemovedFreeItemsList() as $removedFreeItems ) {
			...
			echo Frontend::wdp_get_template( "removed-free-cart-item-stub.php", array(
				'hash'    => $hash,
				'cartUrl' => $cartUrl,
				'qty'     => $qty,
	...
	public function insertRemovedFreeMiniCartItemStubInCart() {
		...
		foreach ( $this->cart->get_context()->getCustomer()->getRemovedFreeItemsList() as $removedFreeItems ) {
			...
			echo Frontend::wdp_get_template( "removed-free-mini-cart-item-stub.php", array(
				'hash'    => $hash,
				'cartUrl' => $cartUrl,
				'qty'     => $qty,
				'options' => $this->context->get_settings()->getOptions(),
	 ...
	 */
	public function handleFreeCartItemRemove( $cart_item_key, $wcCart ) {
		if ( ! $this->cart ) {
			return;
		}

		$cartItem = $wcCart->cart_contents[ $cart_item_key ];
		$facade   = new WcCartItemFacade( $this->cart->get_context()->getGlobalContext(), $cartItem, $cart_item_key );
	 ...
	 * @param \WC_Cart $wcCart
	 */
	public function handleFreeCartItemUpdate( $cartItemKey, $quantity, $oldQuantity, $wcCart ) {
		if ( ! $this->cart ) {
			return;
		}
