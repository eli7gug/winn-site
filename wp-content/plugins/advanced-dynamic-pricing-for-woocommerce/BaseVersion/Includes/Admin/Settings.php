...
//		add_action( 'admin_notices', array ($this, 'notify_coupons_disabled'), 10 );
	...
	public function edit_rules_panel() {
		...
		$list_rules_url_args = array(
			'product'      => $product->get_id(),
			'action_rules' => 'list',
		);

		if ( $product instanceof \WC_Product_Variable && !empty( $product->get_children() ) ) {
			$list_rules_url_args['product_childs'] = $product->get_children();
		}

		if ( !empty( $product->get_sku() ) ) {
			$list_rules_url_args['product_sku'] = $product->get_sku();
		}

		$categories = get_the_terms( $product->get_id(), 'product_cat' );
		if ( $categories !== false && ! ( $categories instanceof \WP_Error ) ) {
			if ( ( $term_ids = array_column( $categories, 'term_id') ) && ! empty( $term_ids ) ) {
				$list_rules_url_args['product_categories'] = $term_ids;
			}
			if ( ( $slugs = array_column( $categories, 'slug') ) && ! empty( $slugs ) ) {
				$list_rules_url_args['product_category_slug'] = $slugs;
			}
		}
		...
		if ( !empty( $product->get_attributes() ) ) {
			...
			foreach ( $product_attributes as $attr ) {
				...
				if ( $terms !== false && ! ( $terms instanceof \WP_Error ) ) {
					$product_attribute_ids = array_merge( $product_attribute_ids,
						Helpers::get_product_attributes( array_column( $terms, 'term_id' ) ) );
				}
			}
			if ( ! empty( $product_attribute_ids ) ) {
				$list_rules_url_args['product_attributes'] = array_column( $product_attribute_ids, 'id');
			}
		}

		$tags = get_the_terms( $product->get_id(), 'product_tag' );
		if ( $tags !== false && ! ( $tags instanceof \WP_Error ) && !empty( $tags ) ) {
			$list_rules_url_args["product_tags"] = array_column( $tags, 'term_id');
		}

		$list_rules_url = add_query_arg( $list_rules_url_args, menu_page_url( 'wdp_settings', false ) );
		...
		if ( isset( $list_rules_url_args['product_childs']) ) {
			$rules_args['product_childs'] = $list_rules_url_args['product_childs'];
		}

		if ( isset( $list_rules_url_args['product_sku'] ) ) {
			$rules_args['product_sku'] = $list_rules_url_args['product_sku'];
		}

		if ( isset( $list_rules_url_args['product_categories'] ) ) {
			$rules_args['product_categories'] = $list_rules_url_args['product_categories'];
		}

		if ( isset( $list_rules_url_args['product_category_slug'] ) ) {
			$rules_args['product_category_slug'] = $list_rules_url_args['product_category_slug'];
		}

		if ( isset( $list_rules_url_args['product_attributes'] ) ) {
			$rules_args['product_attributes'] = $list_rules_url_args['product_attributes'];
		}

		if ( isset( $list_rules_url_args['product_tags'] ) ) {
			$rules_args['product_tags'] = $list_rules_url_args['product_tags'];
		}
	...
	public static function get_filter_titles( $filters_by_type ) {
		...
		foreach ( $filters_by_type['giftable_categories'] as $id ) {
			$result['giftable_categories'][ $id ] = Helpers::get_category_title( $id );
		}
