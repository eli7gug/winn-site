...
class Helpers {
	...
	public static function get_product_custom_fields( $id ) {
		global $wpdb;
		...
		$wp_fields = $wpdb->get_results(
			$wpdb->prepare("SELECT DISTINCT CONCAT(fields.meta_key,'=',fields.meta_value) FROM {$wpdb->postmeta} AS fields
									...
									WHERE products.post_type IN ('product','product_variation') AND products.ID = %d ORDER BY meta_key", $id ), ARRAY_N);

		return array_map( function ( $custom_field ) {
			return array(
				'id'   => current( $custom_field ),
				'text' => current( $custom_field ),
			);
		}, $wp_fields );
	}
